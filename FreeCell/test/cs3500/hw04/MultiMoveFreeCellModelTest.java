...
/**
 * Test class for IFreeCellModel with multi-move.
...
public class MultiMoveFreeCellModelTest {
  ...
  public void testMoveMultiple() {
    ...
    // reset game with new initial conditions
    model.startGame(model.getDeck(), 4, 2, false);
    gameState = "F1:\n" +
            "F2:\n" +
            "F3:\n" +
    ...
    /*
    move valid build to pile which is invalid build,
    but cards form valid build with top card in pile
     */
  ...
  @Test(expected = IllegalArgumentException.class)
  public void moveTooMany() {
    //test initial conditions
    model.startGame(this.validBuildsDeck, 8, 2, false);
    String gameState = "F1:\n" +
            "F2:\n" +
            "F3:\n" +
            "F4:\n" +
    ...
    // test move valid build to valid build, but too many cards at once
    model.move(PileType.CASCADE, 4, 0, PileType.CASCADE, 0);
  }
  ...
  public void moveInvalidBuild() {
    ...
    // move invalid build to location where cards would otherwise be able to go
    model.move(PileType.CASCADE, 2, 11, PileType.CASCADE, 0);
  }

  @Test(expected = IllegalArgumentException.class)
  public void moveToInvalidBuild() {
    //test initial conditions
    model.startGame(this.validBuildsDeck, 8, 2, false);
    String gameState = "F1:\n" +
            "F2:\n" +
    ...
    // test moving a valid build to create an invalid build
    model.move(PileType.CASCADE, 5, 0, PileType.CASCADE, 0);
  }
  ...
  /**
   * Constructor which creates a model factory and initializes the model.
   */
